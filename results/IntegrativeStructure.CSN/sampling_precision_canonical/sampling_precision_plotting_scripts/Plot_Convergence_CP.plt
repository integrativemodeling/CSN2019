...
stats sprintf("%s.Cluster_Population.txt", ARG1) usi 1 prefix "A"
minx = 0
maxx = A_max + 1.75
set xr [minx:maxx]
...
stats sprintf("%s.Cluster_Population.txt", ARG1) usi 2 prefix "N1"
...
set output sprintf("%s.Cluster_Population.pdf", ARG1)
plot sprintf("%s.Cluster_Population.txt", ARG1) usi ($1+0.5):2:xtic(sprintf("Cluster %i", $1)) w boxes notitle lc rgb "#EB7262", \
