...
#############################################
########Written by Ignacia Echeverria #######
######## Adapted by Ilan E. Chemmama ########
...
class get_distance_metrics(object):
    ...
    def compute_DRMSD_all_versus_all(self):
        i = 0
        ntot = len(list(itertools.combinations_with_replacement(self.all_coords.values(), 2)))
        for particles1, particles2 in itertools.combinations_with_replacement(self.all_coords.values(), 2):
    ...
    def _get_rmfs_cluster(self):
        ...
        for mod in open(self.clustering_dir+'/cluster.'+str(self.cluster)+'.sample_B.txt','r'):
            ...
            try:
                ...
                print('Model missing: ', vals)
        rmfs = []
        rmfs.append('../../Cop9_%s/prefilter/cluster.0/cluster_center_model.rmf3' % self.ilan)
        return rmfs
...
for dir in dirs:
    os.mkdir('dRMS_%s_vs_CSNc' % dir)
    os.chdir('dRMS_%s_vs_CSNc' % dir) 
