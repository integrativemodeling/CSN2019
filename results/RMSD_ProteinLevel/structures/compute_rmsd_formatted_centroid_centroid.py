...
class RMSDCalculator(object):
    ...
    def _get_rmfs_cluster(self):
        ...
        '''
        rmfs = []
        rmfs.append(self.clustering_dir)
        return rmfs
...
dirs = ['BMS', 'DHS', 'DSS', 'BMS.DHS', 'DSS.DHS', 'DSS.BMS', 'crystal', 'DSS.BMS.DHS.P']
for dir in dirs:
    os.mkdir('RMSD_CSNc_vs_%s' % dir)
    os.chdir('RMSD_CSNc_vs_%s' % dir) 

    D = RMSDCalculator('../Cop9.%s.rmf3' % dir,
                       0,
                       align_to = alignment,
                       number_of_models = 0)
