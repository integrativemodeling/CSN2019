...
dir0=DSS_BMS_DHS
for dir in BMS DHS DSS BMS_DHS DSS_BMS DSS_DHS DSS_BMS_DHS_P;
do
    for i in `seq 0 7`;
    do
	for j in `seq $i 7`;
	do
	    ...
	    cent1=$(cat structures/RMSD_CSNc_vs_$(echo $dir | sed 's/_/./g')/RMSD_pairs_all_versus_centroid_${i}_${j}.csv | awk '{sum+=$1;b++}END{print sum/b}');
	    
	    echo ${dir} RB$i RB$j $mean1 $devi1 $mean2 $devi2 $cent1;
	done;

    done > Results.${dir}.vs.CSNc.txt;
    
    cat Results.${dir}.vs.CSNc.txt | \
	sed 's/RB7/CSN8/g' | \
	sed 's/RB6/CSN3/g' | \
	sed 's/RB5/CSN2/g' | \
	sed 's/RB4/CSN1/g' | \
	...
	sed 's/RB1/CSN5/g' | \
	sed 's/RB0/CSN4/g' | \
	sort -n -k1 -k2 > Results.${dir}.vs.CSNc.Rbs.txt
    rm Results.${dir}.vs.CSNc.txt;
done
